  ...
  return (
    <div style={{ height: "100%", display: "flex", flexDirection: "column", overflow: "hidden" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 12, padding: "20px 34px", borderBottom: "1px solid var(--color-neutral-900)" }}>
        ...
        <span style={{ fontFamily: "var(--font-heading)", fontSize: 17 }}>{v.title}</span>
      ...
      <div style={{ flex: 1, display: "grid", gridTemplateColumns: "minmax(0, 1fr) 280px", minHeight: 0 }}>
        <div style={{ padding: "22px 34px", display: "flex", flexDirection: "column", gap: 16, overflow: "auto" }}>
          <span style={{ fontSize: 11, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--color-neutral-600)" }}>{v.transcriptLabel}</span>
          {segments.length === 0 ? (
            <p className="secondary-text">{v.waiting}</p>
          ) : (
            segments.map((s, i) => (
        ...
        <div style={{ borderLeft: "1px solid var(--color-neutral-900)", padding: "22px 20px", display: "flex", flexDirection: "column", gap: 18, background: "var(--color-bg)" }}>
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <span style={{ fontSize: 11, letterSpacing: "0.07em", textTransform: "uppercase", color: "var(--color-neutral-600)" }}>{v.infoLabel}</span>
            {source ? <span className="tag tag-outline">{source === "real" ? v.sourceReal : v.sourceMock}</span> : null}
