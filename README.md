...
Le Node.js n'appelle plus aucun fournisseur IA directement : il passe par le service FastAPI (`AI_SERVICE_URL`), qui centralise les integrations (Mistral pour la transcription, la generation du CR et la classification ; Groq pour la moderation ; Vexa pour le bot de reunion). **Le mock n'est plus un mode par defaut** : il ne sert que de filet de securite si une cle API est absente ou si un appel echoue (timeout, quota, format de reponse inattendu) — jamais comme chemin nominal. Les ecrans participant (consentement, acces au CR sans compte, droits RGPD) sont inclus.
...
**Point de vigilance conformite** : l'ancienne integration Kimi K3 (Moonshot AI, hors UE) a ete retiree — la transcription, la generation du CR et la classification passent desormais toutes par Mistral, coherent avec l'argumentaire "stack souveraine" (voir `Stack_Technique_Souveraine.md` et `Analyse_RGPD_Ethique_IA.md` section 5.5). Reserve encore ouverte pour gpt-oss-safeguard-20b tant qu'il est servi par Groq plutot que par une infra UE — le modele est a poids ouverts (Apache 2.0) et pourrait etre auto-heberge le jour ou une infra UE est disponible. Voir le commentaire dans `ai-service/clients/safeguard.py`.

Les integrations sont ecrites au meilleur effort a partir de leur documentation publique (verifiee le 2026-07-22, cf. commentaires dans le code) : Vexa en particulier documente elle-meme un ecart entre son API cloud managee (version 0.10) et sa doc publique (version 0.12) — a reverifier au moment de l'usage. Voir `ai-service/clients/*.py`.
...
2. Renseigner egalement `MISTRAL_API_KEY` et `GROQ_API_KEY` dans `.env.local`.
...
8. Cliquer "Terminer la reunion" : ca appelle `DELETE` du bot Vexa, lance la moderation (gpt-oss-safeguard-20b via Groq) puis la generation du CR (Mistral).
...
ai-service/
  ...
  clients/                     voxtral.py, mistral_cr.py, classifier.py, safeguard.py (Groq), vexa.py — un fournisseur par fichier, fallback mock inclus
